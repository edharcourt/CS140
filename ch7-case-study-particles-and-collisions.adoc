...
The particle simulation runs at _one frame per loop iteration_. This is enormously fast. On my laptop 1000 loop iterations takes about 1 second. That would amount to 1000 FPS. The problem is that we do not know how long a loop iteration takes to execute. So we do not have a consistent frame rate. 
