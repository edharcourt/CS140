...
Computer vision, medical imaging (MRIs, x-rays, CT scans) are all area related to _image processing_. All of your smartphones have image processing software to related to the camera, and the fingerprint sensor on the device has image processing software for fingerprint matching. The software I am using to write these notes has image processing capabilities to resize images.
