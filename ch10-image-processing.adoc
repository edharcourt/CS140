...
One important and common application in computer science is _image processing_. Computer vision, facial recognition, medical imaging (MRIs, x-rays, CT scans) are all part of image processing. All of your smartphones have image processing software; the camera provides a software zoom, and the fingerprint sensor on the device has image processing software for fingerprint matching. Web browsers resize images, and software such as Photoshop is used to do some amazing effects with images.
