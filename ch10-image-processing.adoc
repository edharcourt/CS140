...
The beautiful thing about writing this as a fuction is that if we wanted to filter the image twice we can just use _function composition_ and call `mean filter` two or ever three times.
...
The following program hides a message in an image puts it all together.
...
Comparing tuples like this is handy, but if you didn't know about tuple equality you could do this another with _logical and_.
