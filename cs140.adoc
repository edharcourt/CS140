...
Why are longer passwords better than shorter ones?  In the English alphabet lets say we allowed passwords to contain 26 upper or lower case characters (that is 52 possibile alphabetic characters), ten digits 0 through 9, and 32 symbol characters `~!@#$%^&*()_-+={[}]"':;>.<,?/|\`. That is 94 possible distinct characters that can be used in a password.  
...
So what are total number of passwords up to 10 characters long? We would have to include all of the nine character passwords, and the eight character passwords, etc. [formla goes here]
