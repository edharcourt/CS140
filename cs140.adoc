...
The logical operator `or` combines a logical value on the left and the right, _X_ `or` _Y_. The entire logical expression is true if either of _X_ or _Y_ are true.  We often state this in terms of a _truth table_. 
...
if (d1 == 1 or d2 == 1) and (d1 != 1 or d2 != 1): <1>
    print("exactly one, 1")
----
<1> Check the parentheses, they are necessary.
