...
.Check Yourself +++<span style='color:red;margin-right:1.25em; display:inline-block;'>&nbsp;&nbsp;&nbsp;</span>+++
Modify the Brownian particle simulation so that a particle stays within the bounds of the window.
...
Another more interesting solution migh be to pretent that the display _wraps around_. For example, if a particle goes off the right edge it come in  on the left edge, and vice-versa. If a particle goes off the top then it appears at the bottom. 

An obvious way to do this is to use an if-statement to check if the particle has gone off an edge and reset its coordinate accordingly. 
...
Think of the display as a piece of paper where curl the paper around on the x-axis then we curl it again arond the y-axis. We end up with the topological shape of a _torus_, or a donut.
