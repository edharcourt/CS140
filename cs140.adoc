...
The real power in programming comes from how we can combine loops, conditional statements, and functions in creative ways to solve or explore a problem.  We will continue our investigations exploring the nature of _randomness_. Randomness plays an important role in many areads of computer science, including cryptography, networking, and simulation and modeling.

Lets simulate tossing a coin _n_ times counting the number of times a head or tail appears. We will have _n_ be input by the user. Lets start by tossing a coin once and checking if it is heads or tails. We will use a variable `heads` to keep track of the number times heads appeared and a variable `tails` for the number of times tails appeared.
