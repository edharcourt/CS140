...
Lets say we rolled a pair of dice and wanted to check if we rolled two ones (snake eyes).
...
The logical operator `and` combines a logical value on the left and the right. The entire logical expression is true if both are true.  We of ten state this interms of a _truth table_
