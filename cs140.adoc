...
All that is left to do is to update the particle's coordinates by its velocity and do something sensible when it hits a wall. Lets define a move function like we did in the Brownian Motion example. This move function willbe a little more complicated because `dx` and `dy` will change when the particle hits a wall.
...
Lets define a `move` function much like we did with our Brownian Motion example.
