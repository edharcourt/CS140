...
This is an example of _exponential growth_. Notice how the graph explodes at between 8 and 10 characters (was it ever suggested that you make your passwords more than 8 characters long?)
...
To emphasize the difference between say a six character password and a ten character password, let's say a hacker was trying to break into a system by trying all possible passwords. How long might it take? Lets say we had a powerful computer that could try 1 billions passwords per second. 

For a six character password that's stem:[94^6/10^9/60 \approx 11.5] minutes. (there a 3600 seconds in

that would be stem:[5.4 \times 10^{19}/10^6 = 5.4 \times 10^{13}] seconds, or stem:[10^{13}/60 = 1.67 \times 10^{11}] minutes, which is stem:[1.67 \times 10^{11} / 60 = 2.78 \times 10^9] hours, which is stem:[2.78 \times 10^9/24 = 1.16 \times 10^{8}] days, or 316 thousand years ... approximately.
...
Exponential growth arises in more subtle ways. In the previous example we were raising, the base of the exponent, _94_, to a power.  What if the base were small? Lets take an exampe of world population growth, estimates to be _1.11_% per year in 2017. What does that mean?

Let stem:[t] be the current year current population stem:[P] at thebeginning f year stem:[t] is stem:[P_t]. So stem:[P_{t+1} = P_t + 0.011P_t] or, factoring out the stem:[P_t] then stem:[P_{t+1} = P_t(1.011)]. 

If we looks at plots of over a 10 year (top left), 100 year (top right), 1000 year (bottom left), and 5000 year (bottom right) time horizon the ten year plot looks nearly linear. But as we progress up to 5000 years we see a curve that definitely exponential.
...
One way of interpreting the formula stem:[P_{t+1} = P_t(1.011)] is to see that _the new value of stem:[P] is the old value of stem:[P] times 1.011_. We have seen thispattern many times before in programming. It is just an assignment statement:

 P = P * 1.011

Throwing this in a loop  
