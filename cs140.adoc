...
INFO: The _bounding box_ is the rectangle that circumscribes a geometric shape such as an ellipse, circle, or polygon. 
...
If an image is really just a bunch of integer values it means we can modify the pixel values in some way to come up with a new image. For example, what if we were to decrease each RGBA component of every pixel by 10%? Theoretically this should make the image darker.  
...
Processing a rectangular table of numbers is extremely common in computer science. We have a systematic way in which we can go through each number using two nested for loops. For example, lets say we wanted to produce the following _9 X 9_ multiplcation table on the console:
...
for i in range(1,10):
    ...
    print()
----
<1> The character `\t` represent the tab character. The third argument `end=""` is what we call a _named parameter_ in Python. The `print` function takes an option argument named `end` that indicates how we should handle the end of line. In this case we don't want the `print` statement to print on a separate line, so we suppress printing `\n` with `end=""`.

INFO: A _matrix_ is a rectangular table of numbers.

TIP: The branch of mathematics that deals with matrices is called _linear algebra_.
...
Here's a program that darkens the image below by 10%. We will review it line by line.
