...
<7> If it is then we need to get the corresponding pixel in the image and ...
<8> Make the red component odd.
<9> A new pygame function that saves a surface (in this case `image`) in a new file (in this case `new_image.png`).
...
To tackle the complexity lets try and break this down into some functions. For step 1 lets write a function `randomly_populate` that takes a surface and randomly assigns each cell to be one of green, blue, or empty (white) with equal probability. (We could also modify this so that one color is less populated than another).
...
The function `randomly_populate` filles a surface with blue, green, or empty (white) agents (pixels).
