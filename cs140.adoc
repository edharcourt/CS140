...
We could, write nine separate `get_at` calls to get the color of each pixel, but being a little bit clever we can use a loop. If stem:[(x,y)] represents the center pixel in the nine, then the row above is stem:[y-1] and the row below is stem:[y+1]. The column to the left is stem:[x-1] and the column to the right is stem:[x+1]. We can view this is a small _3 X 3_ grid and use our nested `for`-loop pattern.
