...
will create a new empty surface `surf` with width `w` and height `h`. 
...
Lets say the orginal image was 600 pixels wide and the scaled image is 300 pixels wide. The _x_ coordinate 10 in the scaled image shuold correspond to _x_ coordinate 20 in the original image because the ratio of the widths in the original image to the scaled image is 2-to-1.  
