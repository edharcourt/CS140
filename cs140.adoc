...
.Fox
image::png/fox.png[width=450]

CAUTION: Notice that when you right click on the fox image and save it, that it is a PNG file. Make sure to keep it as a PNG file.
...
<3> Addint the `update` here will update the display after every row is modified. What would happen if we indented this line one level to be inside the inner `for`-loop?
<4> This delay is for dramatc effect only.
