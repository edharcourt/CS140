...
scaled_image = scale(image, image.get_width()//3,   
                     image.get_height()//3)   <1>

win = pygame.display.set_mode((image.get_width(), image.get_height()))  <2>

win.blit(image, (0,0))  <3>

win.blit(scaled_image, (win.get_width()//2 - scaled_image.get_width()//2,
                        win.get_height()//2 - scaled_image.get_height()//2)) <4>

pygame.display.update()
util.wait_for_click()
----
<1> scale the original image
<2> Create a display surface the original size f the image
<3> Blit the original image in the display
<4> Blit the scaled image centered in the display.
