...
With function `randomly_populate`, `count_like_neighbors`, and `find_empty` we can put together the main program for the Schelling Segregation model. If we make the grid too large then the siulation can take a long time to run, so we will kep it at a sqaure greed 400 on a side.
    ...
    for y in range(grid.get_height()):                   <2>
        for x in range(grid.get_width()): 
            ...
            if (r,g,b) != color.white:                 
                ...
                if like < threshold:                     <5>
                    (ex,ey) = find_empty()
                    grid.set_at((ex,ey), (r,g,b))        <6>
                    grid.set_at((x,y), color.white)      <7>
                    moved = moved + 1                    <8>
