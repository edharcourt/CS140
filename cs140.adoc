...
.Check Yourself +++<span style='color:red;margin-right:1.25em; display:inline-block;'>&nbsp;&nbsp;&nbsp;</span>+++
Write a logical condition that is true of the particle as coordinate `(x,y)` is on the display window. Assume that variable `w` and `h` refer to the width and height of the display. 

[.result]
Well `x` must be between `0` and `w` (including 0, but exclusing `w`) and
`y` must be between `0` and `h` (including 0, but excluding `h`).
====
[source,python]
----
(0 <= x and x < w) and (0 <= y and y < h) <1>
----
The parenthese are strictly not necessary but aid in clarity. 
====
