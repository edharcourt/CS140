...
----
<1> We load the image _before_ we construct the display so we know how largeto make the display.
<2> Construct a display srface that is the same size as the image.
<3> We call the `convert_alpha()` method separately _after_ we load the image on line 4 and then construct the display on line 5.  This is becausr the the `convert_alpha` function is making the format of the image compatible with the format of the display.
