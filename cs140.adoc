...
INFO: A _computer simulation_ is a computer program that represents a simulation (or model) of some realy world process. 
...
print(chips) <2>
----
<1> The only change is to remove the condition that the loop terminate stop when `chips` reaches 2000.
...
draws a pixel at coordinate `(x,y)` with the  color `RGB` (which should be an RGB triple).




The Gamler's Ruin is an example of a _random walk_.
