...
[width="25%", options="header", align="center"]
.Logical `not`
|===
| X | `not` X
...
The real power in programming comes from combining loops and conditional statements in (if-statements). Lets simulate tossing a coin _n_ times counting the number of times a head or tail appears. We will have _n_ be input by the user. Lets start by tossing a coin once and checking if it is heads or tails. We will use a variable `heads` to keep track of the number times heads appeared and a variable `tails` for the number of times tails appeared.
