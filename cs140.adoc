...
We can use the formula for the distance between two points to see if the particles are closer than two radii. 
...
But how should we handle the collision? Physics tells us that the vlocity of `ball1` will get transferred to `ball2` and the velocity of `ball2` will get transferred to `ball1`. That is, the two particles _swap_ velocities.
