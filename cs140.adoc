...
window to kill the program. We can check for the `QUIT` event which is named `pygame.QUIT`. Events can come be generated quickly. Just moving your mouse across the pygame display can screen generate hundreds of `pygame.MOUSEMOTION` events. 

Events get stored in the _event queue_. A _queue_ is another name for a line. In the U.S. people might stand in a line to get a movie ticket, but in the U.K. people stand get in a queue. So an _event queue_ is python object that holds events in the order they occur. When you are typing in your favorite word processor you want the characters to appear in the order they are typed! Anything else would be strange.  
