...
`s[i]` is the charater at index `i`.  The bracket notation is also used to pick out consecutive portions of a string, what we call a _slice_. The notation `s[x:y]` extracts the potion ofthe string starting at index `x` and up to but not including the character at index `y`. Lets see we have 
...
Leaving off the ending index then it is assumed the slice is to the end of the string.
