...
has clicked on the X. The pygame function `pygame.event.get()` retrieves the event queue (which is a sequence).
...
=== Pong
https://en.wikipedia.org/wiki/Pong[Pong] is a classic computer game, one of the first computer video games. Lets add a paddle to our particle simulation and bounce the particles off the paddle. First we'll need a graphic for the paddle.
