...
NOTE: We will often use the term _particle_ to represent any kind of moving object such as a baseball, a hockey puck, an electron, etc.
...
while True:
	...
	# draw the aprticle on the screen
...
You'll ntice that the velocity variables `dx` and `dy` are set to be very small. `.04` and `.01`. This goes against our intuition a little in that we are moving the particle four hundredths of a pixel in the _x_ direction and one hundreth of a picxel in the _y_ direction? Can we have fractions of a pixel? Well, no. But if we were to set these values to say 4 and 1, the loops executes to fast that the particle looks like it is just jumping around the screen and not moving fluidly. We could add a delay in the loop body using `pygame.time.delay`, but for reasons that will be come clearer later we don't want to force the loop to execute more slowly, and we want a more general solution for different speed computers, say laptops vs desktops, etc. 

=== Understnading the _Frame Rate_
...
# a clock object for handling the frme rate
clock = pygame.time.Clock()
...
Adding a fourth and fifth particle is a lot of cutting-and-pasting. This should feel wrong. And it is. We'll have a more general solution when we discuss _lists_ where we can have as many particles as we want.

=== Exercises

.Inner Wall
Add an inner wall to the particle simulation that us centered within the display and has a width that is one tenth the width of the window and half of the height of the window. Have a particle bounce off of the left and right face of the wall. Don't worry about the top and bottom. If it happens to hit the top it will look a little weird.
...
will print the integers 0 through 9. More generally, the `for` loop has the sytnax
...
Where `some_sequence_type` represents a sequence of values, such as the sequence of integers generated by the `range` function. The variable `var` will take on each value in the sequence, and `var` cn be used in the loop body, but var should not be modified in the loop body.

We don't also have to use the `range` function to generate a sequence. A string literal is  also a sequence of characters. For example, the loop
...
NOTE: An _event_ is a pygame object that represents some sort of user action such as pressing a key or moving a mouse. An event has data associated with it such as the kind of event; a mouse movement, mouse button press, key press, etc. Additional information depdns on the event. For example, if the mouse was moved then the event also contains the current location of the mouse.

First we need to take a quick side trip and introduse a new kind of python loop.
