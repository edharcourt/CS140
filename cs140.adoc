...
These results were pretty surprising. If fewer than 33% of an agent's neighbors are like them then the agent moves with the end result being a fair amount of segregation.

One thing to check with any model is to see if it matches our intuition. If we lower the threshold, that is we make the agent more tolerant, then we should see less segregation. This is indeed the case.
