...
for word in words:                <2>
    ...
    if guess == word:
        ...
        print("Incorrect. The word is", '"' + word + '"')
----
<1> The variable that referes to the file object does not have to be named `f`, in this case we are calling it `words`.
<2> And the `for`-loop variable does not have to be named `line`, in this case `word` makes sense.
<3> This is a little tricky. We need to remove the end of line character `'\n'`, but the last line in the file doesn't havean end of line chracter but an _end of file_ character.
