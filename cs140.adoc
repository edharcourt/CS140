...
When you add this to the while loop you will notice that you need to keep hitting the up or down arrow key just to move the paddke one pixel. What we would like to have happen is that when we keep holding the key down we continue to generate `KEYDOWN` events at a regular interval. We can do this at the start of the program using the pyagem function
...
What this says is to generate an initial event after one millisencond and then as the key is held down generate an event every millisecond after that. Moving the paddle one pixel at a time is still kind of slow, so we'll set up another variable `paddle_dy` to move it eight pixels every time.
